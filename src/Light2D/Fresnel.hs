module Light2D.Fresnel (
    generateFresnelImage, 
    dumpFresnelFile
) where
...
scene x y = a .|. (b .&. c) where
    a = Result (circleSDF x y 0.5 (-0.5) 0.05) 20.0 0.0 0.0
    b = Result (circleSDF x y 0.5 0.2 0.35) 0.0 0.2 1.5
    c = Result (circleSDF x y 0.5 0.8 0.35) 0.0 0.2 1.5
...
generateFresnelImage :: Image PixelRGB8
generateFresnelImage = generateImage pixelFunc width height where
...
dumpFresnelFile :: IO ()
dumpFresnelFile = writePng "img/fresnel.png" generateFresnelImage
